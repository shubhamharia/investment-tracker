...
from app import Platform, Security, Transaction, PortfolioService
    ...
    for security in securities:
        try:
            ...
            if earliest_transaction:
                ...
                from app import import_historical_data
...
def import_data():
    try:
        ...
        # Read CSV data
        df = pd.read_csv('/app/data/combined_transactions_updated.csv')
...
if __name__ == "__main__":
    if len(sys.argv) > 1 and sys.argv[1] == "import":
        ...
        with app.app_context():
            ...
            # Import CSV data
            csv_file = '/app/data/combined_transactions_updated.csv'
...
def main():
    ...
    with app.app_context():
        if args.command == 'import':
            csv_file = args.csv_file or '/app/data/combined_transactions_updated.csv'
            if os.path.exists(csv_file):
                imported, errors = import_csv_data(csv_file)
                if imported > 0:
                    PortfolioService.calculate_holdings()
